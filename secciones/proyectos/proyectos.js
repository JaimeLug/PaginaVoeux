...
window.addEventListener('section-loaded', (e) => {
    ...
    if (portfolioContainer && typeof PROJECTS_DATA !== 'undefined') {
        ...
        const projectsArray = Object.values(PROJECTS_DATA);
        projectsArray.sort((a, b) => (a.order || 99) - (b.order || 99));
    ...
    <div class="project-card ${spanClass}" data-media-slug="${project.mediaSlug || project.id}" onclick="window.location.hash='#proyecto?id=${project.id}'" aria-label="Ver proyecto ${project.title} de ${project.brand}" role="button" tabindex="0">
        <img ${project.coverPositionY ? `style="object-position: 50% ${project.coverPositionY};"` : ''} src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 9' fill='%23111'%3E%3C/svg%3E" alt="${project.title}" class="project-img">
